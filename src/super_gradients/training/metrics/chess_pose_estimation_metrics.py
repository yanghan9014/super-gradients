...
__all__ = ["ChessPoseEstimationMetrics"]


@register_metric(Metrics.CHESS_POSE_ESTIMATION_METRICS)
class ChessPoseEstimationMetrics(Metric):
    """
    ...
    >>> from super_gradients.training.metrics import ChessPoseEstimationMetrics
    ...
    >>> from super_gradients.training.metrics import ChessPoseEstimationMetrics
    >>> metric = ChessPoseEstimationMetrics(iou_thresholds_to_report=[0.5, 0.75], ...)
    >>> metric.update(...)
    ...
    def update(
        ...
        passing groundtruth information (gt_joints, gt_bboxes, gt_areas) and a new style of passing
        ...
        """
        predictions: List[ChessPoseEstimationPredictions] = self.post_prediction_callback(preds)  # Decode raw predictions into poses
    ...
    def _update_with_old_style_args(
        ...
        The now deprecated way of passing groundtruth information was through a dictionary with 'gt_joints', 'gt_bboxes', 'gt_areas' keys
    ...
    def update_single_image(
        ...
        predicted_class_scores: Union[Tensor, np.ndarray],
        ...
        if len(predicted_poses) == 0 and len(gt_joints) == 0:
            return
        if predicted_class_scores is None:
            raise ValueError("Predicted class scores must be provided for chess pose estimation metrics")
        ...
        gt_labels_tensor = convert_to_tensor(gt_labels if gt_labels is not None else [], dtype=torch.long, device="cpu")
        if gt_labels_tensor.ndim > 1:
            gt_labels_tensor = gt_labels_tensor.view(-1)
        ...
        if len(predicted_scores) == 0 and len(gt_labels_tensor) == 0:
            return
        ...
        for class_id in classes:
            ...
            empty_crowd_targets_areas = torch.zeros((0,), dtype=torch.float32, device="cpu")
            empty_crowd_targets_bboxes = torch.zeros((0, 4), dtype=torch.float32, device="cpu")
    ...
    def compute(self) -> Dict[str, Union[float, torch.Tensor]]:
        ...
        if len(predictions) > 0:
            for cls_idx, cls_value in enumerate(classes):
                cls_entries = [x for x in predictions if x[0] == cls_value]
                if len(cls_entries) == 0:
                    continue

                preds_matched_list = [x[1].cpu() for x in cls_entries]
                preds_to_ignore_list = [x[2].cpu() for x in cls_entries]
