...
class PoseEstimationPipeline(Pipeline):
    ...
    def _decode_model_output(self, model_output: Union[List, Tuple, torch.Tensor], model_input: np.ndarray) -> List[PoseEstimationPrediction]:
        ...
        for image_level_predictions, image in zip(list_of_predictions, model_input):
            ...
            pred_pose_scores = getattr(image_level_predictions, "pose_scores", None)
            pred_class_probabilities = getattr(image_level_predictions, "class_probabilities", None)
            pred_objectness_scores = getattr(image_level_predictions, "objectness_scores", None)
            ...
            bboxes_np = pred_bboxes.cpu().numpy() if torch.is_tensor(pred_bboxes) else pred_bboxes

            if pred_pose_scores is not None:
                pose_scores_np = pred_pose_scores.cpu().numpy() if torch.is_tensor(pred_pose_scores) else pred_pose_scores
            else:
                # If pose_scores not present, default to 1.0 for all joints
                num_joints = poses_np.shape[1] if poses_np.ndim > 1 else 0
                pose_scores_np = np.ones((len(poses_np), num_joints), dtype=float)

            if labels_np is None:
                labels_np = np.zeros(len(poses_np), dtype=int)

            class_probabilities_np = (
                pred_class_probabilities.cpu().numpy() if torch.is_tensor(pred_class_probabilities) else pred_class_probabilities
            )
            objectness_scores_np = (
                pred_objectness_scores.cpu().numpy() if torch.is_tensor(pred_objectness_scores) else pred_objectness_scores
            )
            ...
            decoded_predictions.append(
                PoseEstimationPrediction(
                    ...
                    objectness_scores=objectness_scores_np,
                )
            )
