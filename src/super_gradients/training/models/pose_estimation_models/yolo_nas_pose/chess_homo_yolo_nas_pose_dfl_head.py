...
class ChessHomoYoloNASPoseDFLHead(BaseDetectionModule, SupportsReplaceNumClasses):
    """
    ChessHomoYoloNASPoseDFLHead is the head used in the Chess YoloNASPose model with homography prediction.
    ...
    It implements:
      ...
      - homography regression predicting 8 DoF parameters.
    """
    ...
    def __init__(
        ...
        homo_inter_channels: int = None,
        homo_regression_blocks: int = None,
    ):
        """
        Initialize the ChessHomoYoloNASPoseDFLHead.
        ...
        :param homo_inter_channels: Intermediate channels for homography regression (defaults to pose_inter_channels).
        :param homo_regression_blocks: Number of conv blocks in the homography branch (defaults to pose_regression_blocks).
        """
        super().__init__(in_channels)

        if pose_conf_in_class_head:
            raise ValueError(
                "pose_conf_in_class_head is not supported in ChessHomoYoloNASPoseDFLHead. "
        ...
        homo_inter_channels = width_multiplier(homo_inter_channels, width_mult, 8)
        ...
        # ----- stems -----
        if self.shared_stem:
            max_input = max(bbox_inter_channels, pose_inter_channels, homo_inter_channels)
            ...
            if max_input != homo_inter_channels:
                self.homo_stem = nn.Conv2d(max_input, homo_inter_channels, kernel_size=1, stride=1, padding=0, bias=False)
            else:
                self.homo_stem = nn.Identity()
            ...
            self.homo_stem = ConvBNReLU(in_channels, homo_inter_channels, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        # ----- homography branch -----
        homo_block = pose_block
        homo_convs = [homo_block(homo_inter_channels, homo_inter_channels) for _ in range(homo_regression_blocks)]
        self.homo_convs = nn.Sequential(*homo_convs)
        self.homo_pred = nn.Conv2d(homo_inter_channels, 8, 1, 1, 0)  # 8 DoF parameters
    ...
    def forward(self, x) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        :return: Tuple of [reg_output, cls_output, pose_regression, pose_logits, homo_params]
            ...
            - homo_params:     [B, 8, H, W]      (8 DoF homography parameters per location)
        """
        x = self.stem(x)
        pose_features = self.pose_stem(x)
        bbox_features = self.bbox_stem(x)
        homo_features = self.homo_stem(x)
        ...
        homo_feat = self.homo_convs(homo_features)
        homo_feat = self.reg_dropout_rate(homo_feat)
        homo_params = self.homo_pred(homo_feat)  # [B, 8, H, W]

        return reg_output, cls_output, pose_regression, pose_logits, homo_params
