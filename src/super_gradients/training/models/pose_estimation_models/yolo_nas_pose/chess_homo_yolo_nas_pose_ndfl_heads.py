...
class ChessHomoYoloNASPoseNDFLHeads(BaseDetectionModule, SupportsReplaceNumClasses):
    def __init__(
        self,
        num_classes: int,
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> Union[YoloNasPoseDecodedPredictions, Tuple[YoloNasPoseDecodedPredictions, YoloNasPoseRawOutputs]]:
        ...
        for i, feat in enumerate(feats):
            ...
            reg_distri, cls_logit, pose_regression, pose_logits, homo_param = getattr(self, f"head{i + 1}")(feat)
            ...
            homo_param_list.append(homo_param.mean(dim=[2, 3]))  # [B, 8, H, W] -> [B, 8]
        ...
        homo_param_list = torch.stack(homo_param_list, dim=0).mean(dim=0)  # [B, 8]
        ...
        pred_homography = homo_param_list.detach().clone() # [B, 8]

        decoded_predictions = pred_bboxes, pred_scores, pred_pose_coords, pred_pose_scores, pred_homography
