...
class ChessYoloNASPosePostPredictionCallback(AbstractPoseEstimationPostPredictionCallback):
    ...
    def __call__(self, outputs: Tuple[Tuple[Tensor, Tensor, Tensor, Tensor], ...]) -> List[ChessPoseEstimationPredictions]:
        ...
        for pred_values in zip(*predictions):
            ...
            # board_pose_scores = pred_pose_scores[board_mask]

            piece_final_coords = piece_final_scores = piece_final_labels = None
            board_final_coords = board_final_scores = board_final_labels = None
            ...
            if piece_bboxes_conf.size(0) > 0:
                ...
                piece_final_scores = piece_cls_conf[idx_to_keep]
                piece_final_labels = piece_cls_label[idx_to_keep]

            # if board_bboxes_conf.size(0) > 0:
            #     unique_board_labels = torch.unique(board_cls_label)
            #     keep_indices = []
            #     for lbl in unique_board_labels:
            #         label_mask = board_cls_label == lbl
            #         label_indices = torch.nonzero(label_mask, as_tuple=True)[0]
            #         label_confs = board_cls_conf[label_indices]
            #         best_local_idx = torch.argmax(label_confs)
            #         keep_indices.append(label_indices[best_local_idx])

            #     keep_tensor = torch.stack(keep_indices) if keep_indices else torch.empty(0, dtype=torch.long, device=board_cls_label.device)
            #     board_final_bboxes = board_bboxes_xyxy[keep_tensor]
            #     board_final_class_scores = board_bboxes_conf[keep_tensor]
            #     board_final_poses = torch.cat(
            #         [
            #             board_pose_coords[keep_tensor],
            #             board_pose_scores[keep_tensor].unsqueeze(-1),
            #         ],
            #         dim=-1,
            #     )
            ...
            non_center_scores = board_cls_conf[~center_mask]
            ...
            log_scores = [s.item() for s in torch.log(scores_sorted.clamp_min(1e-6))]       # [N]
            N = len(labels_sorted)
