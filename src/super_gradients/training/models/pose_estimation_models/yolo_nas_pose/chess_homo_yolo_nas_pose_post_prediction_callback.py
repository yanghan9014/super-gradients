...
from super_gradients.module_interfaces.pose_estimation_post_prediction_callback import ChessHomoPoseEstimationPredictions


class ChessHomoYoloNASPosePostPredictionCallback(AbstractPoseEstimationPostPredictionCallback):
    """
    ...
    @torch.no_grad()
    def __call__(self, outputs: Tuple[Tuple[Tensor, Tensor, Tensor, Tensor], ...]) -> List[ChessHomoPoseEstimationPredictions]:
        """
        ...
        decoded_predictions: List[ChessHomoPoseEstimationPredictions] = []
        for pred_bboxes_xyxy, pred_bboxes_conf, pred_pose_coords, pred_pose_scores, pred_homography in zip(*predictions):
            # pred_bboxes_xyxy [Anchors, 4] in XYXY format
            # pred_bboxes_conf [Anchors, Num Classes] confidence scores [0..1]
            # pred_pose_coords [Anchors, Num Keypoints, 2] in (x,y) format
            # pred_pose_scores [Anchors, Num Keypoints] confidence scores [0..1]
            # pred_homography [8] homography matrix
